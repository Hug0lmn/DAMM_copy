...
async function main(){
    ...
    while (compteur < data.processedSignatures.length){
        ...
        for (const transaction of transactions){
            ...
            if (global_map.has(keys_name)){//If a key already exist then calculate the number of tokens at the end
                const map_value = global_map.get(keys_name);
                ...
                global_map.set(keys_name, map_value)
            } else {//If a key doesn't exist then create a new one  
                mini_array.push(1)
                global_map.set(keys_name, mini_array);
            }
        }
    }
    await delay(50);
    fs.writeFileSync("output_wallet.json", JSON.stringify(Array.from(global_map.entries())));
